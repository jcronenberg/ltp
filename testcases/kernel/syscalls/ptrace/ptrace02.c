 ...
 *
 * Test the functionality of ptrace() for PTRACE_TRACEME & PTRACE_CONT requests.
 * Forked child does ptrace(PTRACE_TRACEME, ...).
 * Then a signal is delivered to the child & verified that parent
 * is notified via wait().
 * After parent does ptrace(PTRACE_CONT, ..) to make the child to continue.
 * Again parent wait() for child to finish.
 * Test passes if child finished normally.
 *
 * Testing two cases:
...
static void do_child(unsigned int i)
{
	...
	if ((ptrace(PTRACE_TRACEME, 0, 0, 0)) == -1) {
		tst_res(TWARN, "ptrace(PTRACE_TRACEME, ...) failed in child");
		exit(1);
	}
...
static void run(unsigned int i)
{
	...
	else if ((ptrace(PTRACE_CONT, child_pid, 0, 0)) == -1)
		tst_res(TFAIL | TERRNO, "ptrace(PTRACE_CONT, %i, 0, 0) failed",
			 child_pid);
