...
static void sig_child_defined_handler(void);
...
static void run(unsigned int i)
{
	...
	if (child_stack == NULL)
		tst_brk(TBROK, "Cannot allocate stack for child");
			...
			if (test_setup() != 0) {
				tst_res(TWARN, "test_setup() failed,"
			...
			/* check return code */
			if (TST_RET == -1) {
				tst_res(TFAIL | TTERRNO, "clone() failed");
			...
			if (WTERMSIG(status))
				tst_res(TWARN, "child exitied with signal %d",
					 WTERMSIG(status));
...
static void setup(void)
{
	...
	if ((sprintf(file_name, "parent_file_%ld", syscall(__NR_gettid))) <= 0)
		tst_brk(TBROK | TERRNO, "sprintf() failed");
}

static void cleanup(void)
{
	if (unlink(file_name) == -1)
		tst_res(TWARN | TERRNO, "unlink(%s) failed", file_name);
}
...
static int test_setup(void)
{
	...
	if (getcwd(cwd_parent, sizeof(cwd_parent)) == NULL) {
		tst_res(TWARN | TERRNO, "getcwd() failed in test_setup()");
		return -1;
	}
	...
	if (fd_parent == -1) {
		tst_res(TWARN | TERRNO, "open() failed in test_setup()");
		return -1;
	}
	...
	if (sigaction(SIGUSR2, &def_act, NULL) == -1) {
		tst_res(TWARN | TERRNO, "sigaction() failed in test_setup()");
		return -1;
	}
...
static int test_FILES(void)
{
	if (close(fd_parent) == -1) {
		tst_res(TWARN | TERRNO, "close failed in test_FILES");
		return -1;
	}
	return 0;
}
...
static int test_FS(void)
{
	...
	if (test_tmpdir == NULL) {
		tst_res(TWARN | TERRNO, "tst_get_tmpdir failed");
		rval = -1;
	} else if (chdir(test_tmpdir) == -1) {
		tst_res(TWARN | TERRNO, "chdir failed in test_FS");
		rval = -1;
	} else {
		rval = 0;
	}
...
static int test_SIG(void)
{
	...
	if (sigaction(SIGUSR2, &new_act, NULL) == -1) {
		tst_res(TWARN | TERRNO, "signal failed in test_SIG");
		return -1;
	}

	/* Send SIGUSR2 signal to parent */
	if (kill(getppid(), SIGUSR2) == -1) {
		tst_res(TWARN | TERRNO, "kill failed in test_SIG");
		return -1;
	}
...
static int modified_FILES(void)
{
	...
	/* close fd_parent */
	if ((close(fd_parent)) == -1)
		tst_res(TWARN | TERRNO, "close() failed in modified_FILES()");
...
static int modified_FS(void)
{
	...
	if ((getcwd(cwd, sizeof(cwd))) == NULL)
		tst_res(TWARN | TERRNO, "getcwd() failed");
